show_debug_message("Social async event received");
...
if (ident == achievement_our_info)
{
    ...
    achievement_load_leaderboard("CgkIvN_kkrwKEAIQBA",1,100,achievement_filter_friends_only);
	//achievement_get_challenges();

    // Okay, so we are now logged-in let's look for any cloud saves
    // cloud_synchronise();           
}
...
else if (ident == achievement_leaderboard_info)
{
    ...
    for(i=0;i<numentries;i++)
    {
        ...
        show_debug_message("Found leaderboard entry " + string(i) + " " + string(global.playername[i]) + " rank " + string(global.playerrank[i]) + " score " + string(global.playerscore[i] )+ " id " + string(global.playerid[i]));
    }
}
...
else if (ident == achievement_pic_loaded)
{
    ...
    show_debug_message("Loaded pic - ID = " + string(playerid) + " " + string(spriteid)  );
    
    if (spriteid != -1)
    {
        for(i=0;i<global.numfriends;i++)
        {
            if (global.friendid[i] == playerid) { global.friendpic[i] = spriteid; }
        }
        
        for (i=0;i<global.numentries;i++)
        {
            if (global.playerid[i] == playerid) { global.playerpic[i] = spriteid; }
        }
    }
}
else { show_debug_message("!! Unknown social event received !! ID = " + string(ident)); }
